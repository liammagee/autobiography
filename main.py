...
async def chat_prompt(prompt, parameters):
    try:
        ...
        reply = await generate_reply(parameters["gpt_settings_character"], client_character, prompt_for_character, messages)
...
async def update_character_instructions():
    ...
    try:
        ...
        rewrite_memory_instruction = eval(f"f'''{parameters["prompt_for_narrator_rewrite_memory"]}'''")
        ...
        dialog_narrator.append({"role": "user", "content": f"{rewrite_memory_instruction}: {prompt_for_character}"})
...
async def summarise_autobiography():
    ...
    try:
        ...
        # write_bio_instruction = parameters["write_bio_instruction"] + f":\n\n '{builder}'."
        write_bio_instruction = eval(f"f'''{parameters["prompt_for_narrator_bio"]}'''")
        messages.append({"role": "user", "content": write_bio_instruction})

        return await generate_reply(parameters["gpt_settings_narrator"], client_narrator, prompt_for_narrator, messages)

    except Exception as e:
        print(f"Error summarising biography: {e}, with {write_bio_instruction}")
        return "Error!"
            ...
            if prompt is not None:
                ...
                if result != "":
                    if len(dialog_character) >= max_size_dialog:
                        ...
                        dialog_character.pop(0)
                    dialog_character.append({"role": "user", "content": prompt})
                    dialog_character.append({"role": "assistant", "content": result})
                    bracket_counter = bracket_counter + 1
                    if make_a_promise:
...
def reload_settings():
    ...
    parameters["prompt_for_narrator_bio"] = load_markdown_content(parameters['prompt_for_narrator_bio'])
    parameters["prompt_for_narrator_rewrite_memory"] = load_markdown_content(parameters['prompt_for_narrator_rewrite_memory'])
